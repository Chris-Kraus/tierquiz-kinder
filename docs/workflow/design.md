...
## Startseite- & Sammlungs-Neuaufbau, Tier-Album-Entfernung (21.08.2026, `ux-design`, konsultiert von `business-analyst`)

**Anlass:** Neue Handoff-Datei "CHANGES-startseite-sammlung.md" ersetzt laut eigenem Kopftext ausdrücklich die Startseiten-/Album-Beschreibungen aus README.md und "CHANGES-sterne-maskottchen.md" (Issues #80–#83, Abschnitt oben). Gilt gegen den noch offenen Branch `feature/mascot-unlock-system` (PR #84, `#80`–`#82` bereits QA-abgenommen darauf, `#83` noch offen) — nicht gegen `main`, das diesen Stand noch nicht hat.

### Wichtigster Punkt zuerst: Das Tier-Sticker-Album entfällt vollständig — eine echte Rücknahme bereits ausgelieferter Funktionalität

Die neue Spezifikation sagt explizit: *"Das Tier-Album (9 Tiere) ist ersatzlos entfallen — inklusive aller Texte, die 'Album' erwähnen."* Das betrifft nicht nur die zuletzt auf 3×3 verkleinerte Fassung aus #80/#82, sondern das **gesamte ursprüngliche Sticker-Album-Feature aus dem ersten Redesign** (Issue #68, PR #78, längst nach `main` gemergt) — Album-Vorschau auf Start und Ergebnis, "NEU!"-Sticker im Feedback-Panel-Konzept, das komplette `src/quiz/album.js`-Modul. Verifiziert im Code: `addCollectedAnimal()` wird aktuell aus fünf verschiedenen Frage-Bildschirmen aufgerufen (Quizfragen, Wer-bin-ich?, Tiergeräusche, Buchstabensuche, Tier-Memory), nicht nur aus start.js/result.js wie man beim ersten Blick vermuten könnte — die Entfernung ist damit größer als "zwei Dateien anpassen". **UX-Bewertung: keine Einwände gegen die Entfernung selbst** — das neue "Meine Sammlung"-Konzept (Maskottchen-Fortschritt) übernimmt die motivierende Sammel-Funktion vollständig und ist sogar direkter an die neue Sterne-Währung gekoppelt als das bisherige, davon losgelöste Tier-Album. Das ist aber eine **produktseitige Entscheidung, kein rein gestalterisches Detail** — siehe `requirements.md` für die explizite Kennzeichnung als Rücknahme.

### Startseite: neuer Zeilenaufbau + Kopfzeile ausgeblendet

Statt des bisherigen zweispaltigen Layouts (Hauptspalte + `.start-screen__side`) verlangt die neue Spezifikation volle Breite mit untereinander gestapelten Zeilen: Titel → Kartenzeile ("Mein Maskottchen" + "Meine Sammlung", `1fr 1.5fr`) → Modus-Auswahl (eine Reihe, s.u.) → Schwierigkeit → Fragenanzahl → CTA. Das ist ein echter struktureller Bruch mit dem bisherigen `start-screen__main`/`start-screen__side`-Grid aus #71/#77, keine reine Nachrüstung.

**Kopfzeile auf der Startseite komplett ausgeblendet** (`showHeader = screen !== "start"`), da das neue Layout dort ohnehin kein Logo/Modus-Pill/Fortschritt braucht (die einzige noch verbleibende Kopfzeilen-Information — das Sterne-Badge — wandert gezielt unter die Sammlungs-Karte, s.u.). **Barrierefreiheits-Einschätzung: kein Einwand, aber ein Verifikationspunkt.** Der bestehende Home-Button (`.app-header__home-button`) liegt aktuell nur in der Kopfzeile — auf der Startseite selbst gibt es aber ohnehin nichts, wohin er führen würde (man ist ja schon dort), sein Fehlen ist also keine Navigations-Lücke. Zu prüfen bleibt für `web-developer`/QA: dass der Tab-Reihenfolge-Sprung vom letzten fokussierbaren Element eines Frage-/Ergebnis-Bildschirms zur ersten Kachel der Startseite (kein Zwischenschritt über eine nicht existierende Kopfzeile) weiterhin sinnvoll bleibt, und dass ein Screenreader-Nutzer beim Wechsel zur Startseite nicht durch eine plötzlich fehlende, vorher immer vorhandene Landmark irritiert wird (`role="banner"`/`<header>`, falls die Kopfzeile aktuell so ausgezeichnet ist) — einmaliger Verifikationsschritt, keine Vorab-Änderung nötig.

### "Mein Maskottchen"-Karte + wiederverwendbare 3-teilige Nav

Ersetzt das bisherige Karussell (kleine Bühne + Kopfzeile mit Pille) durch eine große, quadratische Bühne (`aspect-ratio: 1.1`) plus darunterliegendes Nav-Element (Pfeil ← / weißes Badge "{position}/{anzahl}" / Pfeil →) — inhaltlich dieselbe Funktion (durch freigeschaltete Maskottchen blättern), rein optisch/strukturell neu. **Empfehlung: dieses Nav-Element als eine einzige, wiederverwendbare Komponente bauen, nicht wie bisher pro Screen duplizieren.** Es kommt laut Handoff 4× vor (Start-Maskottchen-Nav, Start-Sammlung-Seiten-Nav, Ergebnis-Maskottchen-Nav, Ergebnis-Sammlung-Seiten-Nav) — das bisherige Projekt-Muster "kleine Duplikate zwischen genau 2 Screens sind okay" (z. B. `renderMascotCarouselMarkup`/`formatStars` zwischen start.js/result.js) war bewusst für den Fall von 2 Kopien gewählt; bei 4 identischen Kopien mit einer sicherheitsrelevanten Detail-Anforderung (Doppel-Tap-Robustheit, s. u.) wird eine gemeinsame Komponente sinnvoller als weitere Vervierfachung — technische Umsetzung/Ablageort liegt bei `software-architect` (siehe `architecture.md`).

**Doppel-Tap-Anforderung (Handoff-Hinweis zu "funktionalen Updatern"): das React-spezifische Detail ist hier gegenstandslos, die zugrunde liegende UX-Anforderung bleibt real.** Der Handoff begründet `setState(st => ...)` mit React-Batching bei schnellen Taps — das Projekt ist Vanilla JS ohne diese Batching-Falle, dieser Punkt ist als N/A zu kennzeichnen (kein Architektur-Nachbau eines React-Musters nötig). Die eigentliche Nutzeranforderung dahinter bleibt aber ein echtes Akzeptanzkriterium: **zwei schnelle Taps auf einen Pfeil müssen zwei Schritte weiterblättern, nicht nur einen.** Das ist bei einem Kind mit noch nicht voll ausgereifter Motorik (siehe "Zielgruppen-Einschätzung") ein realistisches Szenario, kein Rand­fall. Für die Verifikation: manueller Doppel-Tap-Test bei der QA-Abnahme dieser Story, unabhängig davon, welches synchrone Lese-Schreib-Muster `web-developer`/`software-architect` konkret wählen.

### "Meine Sammlung"-Karte (Maskottchen-Sammelraster, ersetzt das Tier-Album konzeptionell)

Neues Konzept, kein Wiederverwenden des alten Album-Rasters: zeigt die 50 Maskottchen selbst (freigeschaltet = Tint + Name, verdeckt = "?"-Platzhalter), 9 pro Seite, 6 Seiten, gleiche wiederverwendbare Nav-Komponente wie oben (Badge zeigt hier "Seite {n}/{gesamt}" statt Positions-Text). **Kachel-Mindestgröße/Kontrast:** 108px Mindesthöhe, 3px/4px Rahmen, "?"-Farbe `#B08A55` auf `#FFF8EC` — Kontrastprüfung empfohlen (analog zur bereits einmal korrigierten `--muted`-Token-Prüfung beim ursprünglichen Redesign), da dieser Farbwert neu ist und noch nicht gegen WCAG AA verifiziert wurde; sollte `web-developer` beim Umsetzen kurz mit einem Kontrast-Checker bestätigen, kein Blocker für den Storystart.

**Stern-Badge auf der Startseite — eigene Variante, kein Duplikat der Header-Logik, aber dieselbe zugrunde liegende Regel.** Da die Kopfzeile auf der Startseite fehlt (s. o.), braucht das Sterne-Badge dort eine eigene Platzierung (mittig unter der Sammlungs-Karte) und eigene Zustands-Optik (unter 5 Sternen: unsichtbar/ohne Fläche, `disabled`; ab 5 Sternen: dunkle CTA-Optik, animiert, klickbar) — inhaltlich identisch zur bereits bestehenden `canRedeem`-Logik aus #81 (`stars >= 5 && unlockedIds.length < 50`), nur mit anderem Markup/Styling für genau diesen einen Ort. **Empfehlung:** die bestehende `canRedeem`-Berechnung aus `header.js` wiederverwenden/exportieren statt ein zweites Mal zu berechnen (Single Source of Truth für die Freischalt-Bedingung) — reine Darstellungs-Variante, keine zweite Logik-Kopie. Barrierefreiheit identisch zum Header-Badge: `<button>`, `aria-label`/`disabled`+`aria-disabled` wie in #81 bereits umgesetzt.

### Modus-Kacheln: Rückkehr zu einer Zeile bei 1280px — bewusste Reversion einer dokumentierten Entscheidung, nicht stillschweigend übernehmen

Der Abschnitt "Modus-Auswahl auf dem Start-Bildschirm: Fünfter/sechster Modus — Skalierungsentscheidung" (16.08.2026, oben) hatte bei 5 Kacheln bewusst einen **zweizeiligen** Umbruch (3+2) empfohlen, u. a. mit der Begründung, Kachelgröße/Touch-Ziele nicht durch Verkleinern zu gefährden. Die neue Spezifikation verlangt jetzt ausdrücklich **eine** Reihe (`grid-template-columns: repeat(5, minmax(0,1fr))`) bei 1280px, mit kleinerem Icon-Quadrat (52×52 statt vorher) und zentriertem Spalten-Layout (Icon oben, Text darunter) statt der bisherigen Zeilen-Anordnung. **UX-Einschätzung: vertretbar, aber ausdrücklich als Reversion zu kennzeichnen, nicht als Weiterentwicklung der alten Entscheidung misszuverstehen.** Bei einer festen Container-Breite von 1280px und `minmax(0,1fr)` bleibt genug Raum pro Kachel (~230px abzüglich Gaps), um die 44×44px-Mindest-Touchfläche komfortabel zu erfüllen — die ursprüngliche Sorge (Kacheln würden bei einer Reihe zu schmal) trifft bei dieser Spaltenbreite nicht zu. Die harte Grenze "6. Modus = eigener Vorschaltbildschirm neu bewerten" aus der alten Entscheidung bleibt unverändert gültig und ist von dieser Reversion nicht betroffen — nur die Ein-vs-Zwei-Zeilen-Frage bei genau 5 Kacheln wird hiermit neu entschieden.

### Ergebnis-Bildschirm: bedingtes Label + rechte Spalte auf "Meine Sammlung" umgestellt

Label wechselt von immer "Runde geschafft 🎉" zu bedingt: nur bei tatsächlich verdientem Stern dieser Runde, sonst "Runde beendet" — vermeidet den bisherigen Widerspruch zum direkt darunterstehenden Satz "Ab 5 richtigen Tieren gibt es einen Stern" bei einer Runde ohne Stern. Rechte Spalte wechselt vom Album (3 Spalten) auf dieselbe "Meine Sammlung"-Karte wie auf der Startseite plus separatem Maskottchen-Nav+Bühne darunter — technisch dieselbe Komponente wie oben, nur zweitplatziert.

### Barrierefreiheit — Zusammenfassung der neuen/geänderten Punkte

- Wiederverwendbare Nav-Komponente: Pfeile mit `aria-label` ("Vorheriges/Nächstes Maskottchen" bzw. "Vorherige/Nächste Sammlungsseite" je nach Kontext, nicht identisches Label an allen 4 Stellen — sonst ist für Screenreader-Nutzer nicht erkennbar, welche der beiden Navs gerade den Fokus hat), `disabled` an den Rändern (bereits etabliertes Muster aus #82).
- Bühne/Sammlungsraster: `aria-live="polite"` bei Seiten-/Positionswechsel (gleiches Muster wie bestehende `.mascot-carousel__stage`).
